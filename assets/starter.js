...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const deleteskill = document.getElementsByClassName("profile-skill-remove");
  ...
  skillInput.addEventListener("keydown", function (event) {
    ...
    if (event.key === "Enter") {
      ...
      // console.log(`test ${typeof(skillValue)}`)

      if (skillValue) {
        const skillAdd = document.createElement("li");
        skillAdd.classList.add("profile-skill-tag");
        skillAdd.dataset.skill = skillValue;

        const span = document.createElement("span");
        span.textContent = skillValue;

        const button = document.createElement("button");
        button.classList.add("profile-skill-remove");
        button.setAttribute("aria-label", `Remove skill ${skillValue}`);
        button.textContent = "✕";
        ...
        skillAdd.appendChild(span);
        skillAdd.appendChild(button);
        profileSkillsList.appendChild(skillAdd);
        skillInput.value = "";
      }
    }

    for (let z = 0; z < allJobs.length; z++) {
      const searchSkills = (skills) => {
        for (let j = 0; j < skills.length; j++) {
          if (skills[j].toLowerCase().includes(skillInput.value)) {
            return true;
          }
        }
        return false;
      };
      if (searchSkills(allJobs[z].skills)) {
        tempjobs.push(allJobs[z]);
      }
    }
    renderJobs(tempjobs);
    statsCounter.innerHTML = `<span style="color:#5B86E5">${tempjobs.length}</span> offres trouvee sur ${allJobs.length}`;
  });
  ...
  const addNewJobList = document.getElementById("save-job-btn");

  let currentJobId = 0;

  console.log(manageModalCloseBtn.value);

  addNewJobList.addEventListener("click", function (e) {
    e.preventDefault();
    currentJobId++;
    const job = {
      id: currentJobId,
      ...
      skills: jobSkillsInput.value,
      description: jobDescriptionInput.value,
    };

    manageJobsList.innerHTML += `
    <li class="manage-job-item" data-job-id="${job.id}">
    <img src="${job.logo}" alt="${job.company} logo" class="job-card__logo" style="position: static; width: 48px; height: 48px; border-radius: 5px;">
  ...
  const createJobCardHTML = (job) => {
    ...
    const tagsHTML = tags
      .map(
        (tag) => `<span class="job-card__tag" data-tag="${tag}">${tag}</span>`
      )
      .join("");
  ...
  const applyAllFilters = () => {
    ...
    searchInput.addEventListener("input", function () {
      ...
      statsCounter.innerHTML = `<span style="color:#5B86E5">${resultJobs.length}</span> offres trouvee`;
    });
  };
  ...
  initializeApp();
  const btn = document.getElementsByClassName("profile-form__save-btn");
